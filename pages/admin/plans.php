...
function plans_page(): void
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'delete') {
            ...
            $pdo->prepare('UPDATE membership_plans SET plan_name = ?, plan_type = ?, duration_days = ?, price = ?, description = ?, is_active = ?, commission_rate = ? WHERE plan_id = ? AND gym_id = ?')->execute([post('plan_name'), post('plan_type'), post('duration_days'), $price, post('description'), post('is_active', 0) ? 1 : 0, post('commission_rate', 5.0), post('id'), $gymId]);
            ...
            $price = (float) post('price');
            $pdo->prepare('INSERT INTO membership_plans (gym_id, plan_name, plan_type, duration_days, price, description, is_active, commission_rate) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')->execute([$gymId, post('plan_name'), post('plan_type'), post('duration_days'), $price, post('description'), post('is_active', 0) ? 1 : 0, post('commission_rate', 5.0)]);

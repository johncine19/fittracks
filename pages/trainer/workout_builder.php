...
function workout_builder_page(): void
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'auto_generate_day') {
            ...
            auto_populate_day($planId, $memberId, $dayOfWeek);
            flash('Day auto-generated.', 'success');
            header('Location: index.php?page=workout_builder&member_user_id=' . $memberId);
            exit;
        }
            ...
            <div class="panel" style="display: flex; flex-direction: column;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px solid var(--line);">
                    ...
                    <div style="display: flex; gap: 5px;">
                        <button onclick="promptAutoGenerateDay(<?= $dayNum ?>, '<?= $dayName ?>')" class="btn" style="padding: 4px 10px; font-size: 12px; background: transparent; border: 1px solid var(--lime); color: var(--lime); border-radius: 4px;">
                            Auto-Gen
                        </button>
    ...
    <script>
    function promptAutoGenerateDay(dayNum, dayName) {
        Swal.fire({
            title: 'Auto-Generate ' + dayName + '?',
            text: "This will replace the exercises for this specific day. Proceed?",
            icon: 'question',
